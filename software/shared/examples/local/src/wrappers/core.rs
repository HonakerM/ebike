...
pub async fn broadcast_message(msg: Message) {
    if let Some(sender) = LOCAL_CAN_SEND.get() {
        match sender.send(msg) {
            Err(err) => {
                eprintln!(
                    "Failed to send message: {:?} most likely due to no recievers",
                    err
                )
            }
            _ => {}
...
static CURRENT_CAR_STATE: OnceLock<Mutex<CarState>> = OnceLock::new();
...
pub async fn state_updater() {
    loop {
        let msg = get_next_message().await;
        {
            ...
            local_mut.update(msg);
        }
    }
}
pub async fn get_ecu_throttle() -> Percentage {
    let mut local_mut = CURRENT_CAR_STATE.get().unwrap().lock().await;
    local_mut.ecu.throttle
}

pub async fn get_car_state() -> CarState {
    let mut local_mut = CURRENT_CAR_STATE.get().unwrap().lock().await;
    local_mut.clone()
}

pub fn setup() -> (Config, tokio::sync::broadcast::Sender<Message>) {
    let (can_send, can_recv) = broadcast::channel(16);
    LOCAL_CAN_SEND.set(can_send.clone()).unwrap();

    CURRENT_OUTSIDE_STATE
        .set(Mutex::new(CurrentOutsideState::default()))
        .unwrap();
    CURRENT_CAR_STATE
        .set(Mutex::new(CarState::default()))
        .unwrap();
